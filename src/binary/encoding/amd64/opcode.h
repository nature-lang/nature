...
typedef enum {
    ...
    OPERAND_TYPE_IMM64,
    OPERAND_TYPE_XMM1,
    OPERAND_TYPE_XMM1S64,
    OPERAND_TYPE_XMM1S32,
    OPERAND_TYPE_XMM1M64,
    OPERAND_TYPE_XMM1M32,
    OPERAND_TYPE_XMM2,
    OPERAND_TYPE_XMM2S64,
    OPERAND_TYPE_XMM2S32,
...
} amd64_opcode_inst_t; // 机器指令,中间表示, 该中间表示可以快速过渡到 inst_format
 ...
 */
typedef struct {
    amd64_opcode_inst_t **list; // 默认初始化 10 大小
    int count;
} amd64_opcode_insts_t;
...
typedef struct {
    amd64_opcode_insts_t insts; // data 数据段，最终的叶子节点才会有该数据
