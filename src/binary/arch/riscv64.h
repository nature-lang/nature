...
static inline void elf_riscv64_operation_encodings(elf_context_t *ctx, slice_t *closures) {
    if (closures->count == 0) {
        return;
    }
            ...
            *temp->offset = section_offset;
            ...
            if (rel_operand != NULL) {
                ...
                if (riscv64_is_call_op(operation) || riscv64_is_branch_op(operation)) {
                    ...
                    riscv64_asm_inst_encoding(operation);
    ...
    for (int i = 0; i < build_temps->count; ++i) {
        ...
        if (sym->st_value > 0) {
            ...
            riscv64_asm_inst_encoding(temp->operation);
