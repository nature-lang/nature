...
static void linear_env_assign(module_t *m, ast_assign_stmt_t *stmt) {
    ...
    push_rt_call(m, RT_CALL_ENV_ASSIGN_REF, NULL, 4, m->linear_current->fn_runtime_operand, index, src_ref,
                 int_operand(size));
}
...
static lir_operand_t *linear_call(module_t *m, ast_expr_t expr, lir_operand_t *target) {
    ...
    // TODO POST_TPL_HOOK
//    if (type_fn->tpl) {
//        push_rt_call(m, RT_CALL_POST_TPL_HOOK, NULL, 1, string_operand(type_fn->name));
//    }
...
static lir_operand_t *linear_fn_decl(module_t *m, ast_expr_t expr, lir_operand_t *target) {
    ...
    for (int i = 0; i < fndef->capture_exprs->length; ++i) {
        ...
        push_rt_call(m, RT_CALL_ENV_ASSIGN, NULL, 4, env_operand, int_operand(ct_reflect_type(item->type).hash),
                     int_operand(i),
                     stack_addr_ref);
    }
...
static void linear_throw(module_t *m, ast_throw_stmt_t *stmt) {
    ...
    push_rt_call(m, RT_CALL_CO_THROW_ERROR, NULL, 5, msg_operand, path_operand, fn_name_operand, line_operand,
                 column_operand);
