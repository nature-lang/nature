...
void debug_interval(closure_t *c) {
    ...
    log_debug("closure=%s interval ------------------------------------------------------------------------", c->symbol_name);
    for (int reg_id = 1; reg_id < cross_alloc_reg_count(); ++reg_id) {
        ...
        log_debug("reg: index(%d-%s), parent(%d-%s), assigned=(%d-%s), stack_slot=%ld, ranges=%s, use_pos=%s", interval->index, reg->name,
                  parent_index, parent_ident, interval->assigned, type_str, stack_slot, ranges, use_pos);
    }
    ...
    for (int i = 0; i < c->var_defs->count; ++i) {
        ...
        log_debug("var: index(%d-%s), parent(%d-%s), assigned=(%d-%s), stack_slot=%ld, ranges=%s, use_pos=%s", interval->index,
                  interval->var->ident, parent_index, parent_ident, interval->assigned, type_str, stack_slot, ranges, use_pos);
    }
    printf("\n\n");
#endif
}
