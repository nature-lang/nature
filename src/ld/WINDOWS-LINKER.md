...
`SOURCES.lock` records the source and tool versions used for the checked-in
CRT, import-library, and third-party archive inputs. `CAPABILITIES.json` is an
optional diagnostic inventory generated by the `coff-capabilities` command; it
is not read by the ordinary linker build path.
