...
typedef struct {
    ...
    linux_elf_symbol_type type;  // fn/var
    bool is_rel; // 是否引用外部符号
    bool is_local; // 是否是本地符号
    linux_elf_section section; // 所在段，text/data
...
void linux_elf_to_file(uint8_t *binary, uint64_t count, string file);
 ...
 * @return
 */
string linux_elf_shdr_build(uint64_t text_size,
                            uint64_t data_size,
