...
void compiler(module_t *m, ast_closure *ast) {
    m->lir_line = 0;
    compiler_closure(m, NULL, ast, NULL);
}
...
// 捕获逃逸变量，并放在形参1中,对应的实参需要填写啥吗？
    ...
    if (parent != NULL && ast_closure->env_count > 0) {
        ...
        for (int i = 0; i < ast_closure->env_count; ++i) {
            ...
            list_append(parent_list, compiler_expr(m, parent, item_expr, expr_target));
    ...
    c->parent = parent;
    slice_push(m->compiler_closures, c);
    ...
    // 编译 body
    list *await = compiler_block(m, c, ast_closure->function->body);
...
list *compiler_block(module_t *m, closure *c, slice_t *block) {
    ...
    for (int i = 0; i < block->count; ++i) {
        ...
        m->lir_line = stmt->line;
#ifdef DEBUG_COMPILER
        debug_stmt("COMPILER", stmt);
#endif
        list *await = compiler_stmt(m, c, stmt);
        list_append(operates, await);
    }
...
list *compiler_stmt(module_t *m, closure *c, ast_stmt *stmt) {
    switch (stmt->type) {
        case AST_NEW_CLOSURE: {
            return compiler_closure(m, c, (ast_closure *) stmt->stmt, NULL);
        }
        case AST_VAR_DECL: {
            return compiler_var_decl(m, c, (ast_var_decl *) stmt->stmt);
        }
        case AST_STMT_VAR_DECL_ASSIGN: {
            return compiler_var_decl_assign(m, c, (ast_var_decl_assign_stmt *) stmt->stmt);
        }
        case AST_STMT_ASSIGN: {
            return compiler_assign(m, c, (ast_assign_stmt *) stmt->stmt);
        }
        case AST_STMT_IF: {
            return compiler_if(m, c, (ast_if_stmt *) stmt->stmt);
        }
        case AST_STMT_FOR_IN: {
            return compiler_for_in(m, c, (ast_for_in_stmt *) stmt->stmt);
        }
        case AST_STMT_WHILE: {
            return compiler_while(m, c, (ast_while_stmt *) stmt->stmt);
        }
        case AST_CALL: {
            return compiler_call(m, c, (ast_call *) stmt->stmt, NULL);
        }
        case AST_STMT_RETURN: {
            return compiler_return(m, c, (ast_return_stmt *) stmt->stmt);
        }
        case AST_STMT_TYPE_DECL: {
            return list_new();
        }
        default: {
            error_printf(m->compiler_line, "unknown stmt");
            exit(0);
        }
    }
}
...
list *compiler_var_decl_assign(module_t *m, closure *c, ast_var_decl_assign_stmt *stmt) {
    ...
    list_append(operates, compiler_expr(m, c, stmt->expr, src));
...
list *compiler_assign(module_t *m, closure *c, ast_assign_stmt *stmt) {
    ...
    list *operates = compiler_expr(m, c, stmt->left, left);

    // 如果 left 是 var
    lir_operand *right = lir_new_empty_operand();
    list_append(operates, compiler_expr(m, c, stmt->right, right));
...
list *compiler_expr(module_t *m, closure *c, ast_expr expr, lir_operand *target) {
    ...
    if (expr.assert_type == AST_EXPR_LITERAL) {
        return compiler_literal(m, c, (ast_literal *) expr.expr, target);
    }
    if (expr.assert_type == AST_EXPR_IDENT) {
        return compiler_ident(m, c, (ast_ident *) expr.expr, target);
    }
    ...
    switch (expr.assert_type) {
        case AST_EXPR_BINARY: {
            return compiler_binary(m, c, expr, target);
        }
        case AST_EXPR_UNARY: {
            return compiler_unary(m, c, expr, target);
        }
        case AST_CALL: {
            return compiler_call(m, c, (ast_call *) expr.expr, target);
        }
        case AST_EXPR_ACCESS_LIST: {
            return compiler_array_value(m, c, expr, target);
        }
        case AST_EXPR_NEW_ARRAY: {
            return compiler_new_array(m, c, expr, target);
        }
        case AST_EXPR_ACCESS_MAP: {
            return compiler_access_map(m, c, expr, target);
        }
        case AST_EXPR_NEW_MAP: {
            return compiler_new_map(m, c, expr, target);
        }
        case AST_EXPR_SELECT_PROPERTY: {
            return compiler_select_property(m, c, expr, target);
        }
        case AST_EXPR_NEW_STRUCT: {
            return compiler_new_struct(m, c, expr, target);
        }
        case AST_EXPR_ACCESS_ENV: {
            return compiler_access_env(m, c, expr, target);
        }
        case AST_NEW_CLOSURE: {
            return compiler_closure(m, c, (ast_closure *) expr.expr, target);
        }
        default: {
...
list *compiler_binary(module_t *m, closure *c, ast_expr expr, lir_operand *result_target) {
    ...
    list *operates = compiler_expr(m, c, binary_expr->left, left_target);
    list_append(operates, compiler_expr(m, c, binary_expr->right, right_target));
 ...
 * @return
 */
list *compiler_unary(module_t *m, closure *c, ast_expr expr, lir_operand *result_target) {
    list *operates = list_new();
    ast_unary_expr *unary_expr = expr.expr;

    lir_operand *first = lir_new_empty_operand();
    list_append(operates, compiler_expr(m, c, unary_expr->operand, first));
...
list *compiler_if(module_t *m, closure *c, ast_if_stmt *if_stmt) {
    ...
    list *operates = compiler_expr(m, c, if_stmt->condition, condition_target);
    ...
    lir_operand *alternate_label_operand = lir_new_label_operand(
            LIR_UNIQUE_NAME(ALTERNATE_IF_IDENT), true);
    ...
    // 编译 consequent block
    list *consequent_list = compiler_block(m, c, if_stmt->consequent);
    ...
    if (if_stmt->alternate->count != 0) {
        ...
        list *alternate_list = compiler_block(m, c, if_stmt->alternate);
...
list *compiler_call(module_t *m, closure *c, ast_call *call, lir_operand *target) {
    ...
    list *operates = compiler_expr(m, c, call->left, base_target);

    if (base_target->type == LIR_OPERAND_TYPE_SYMBOL_LABEL &&
        is_print_symbol(((lir_operand_symbol_label *) base_target->value)->ident)) {
        return compiler_builtin_print(m, c, call, ((lir_operand_symbol_label *) base_target->value)->ident);
    }
    ...
    for (int i = 0; i < call->actual_param_count; ++i) {
        ...
        list *param_list = compiler_expr(m, c, ast_param_expr, param_target);
...
list *compiler_array_value(module_t *m, closure *c, ast_expr expr, lir_operand *target) {
    ...
    lir_operand *base_target = lir_new_empty_operand();
    list *operates = compiler_expr(m, c, ast->left, base_target);
    ...
    list_append(operates, compiler_expr(m, c, ast->index, index_target));
...
list *compiler_new_array(module_t *m, closure *c, ast_expr expr, lir_operand *base_target) {
    ...
    for (int i = 0; i < ast->count; ++i) {
        ...
        list_append(operates, compiler_expr(m, c, item_expr, value_target));
...
list *compiler_access_map(module_t *m, closure *c, ast_expr expr, lir_operand *target) {
    ...
    list_append(operates, compiler_expr(m, c, ast->key, key_target));
...
list *compiler_new_map(module_t *m, closure *c, ast_expr expr, lir_operand *base_target) {
    ...
    for (int i = 0; i < ast->count; ++i) {
        ...
        list_append(operates, compiler_expr(m, c, key_expr, key_target));
        list_append(operates, compiler_expr(m, c, value_expr, value_target));
...
list *compiler_for_in(module_t *m, closure *c, ast_for_in_stmt *ast) {
    lir_operand *base_target = lir_new_empty_operand();
    list *operates = compiler_expr(m, c, ast->iterate, base_target);
    ...
    // block
    list_append(operates, compiler_block(m, c, ast->body));
...
list *compiler_while(module_t *m, closure *c, ast_while_stmt *ast) {
    ...
    lir_operand *condition_target = lir_new_empty_operand();
    list_append(operates, compiler_expr(m, c, ast->condition, condition_target));
    lir_op *cmp_goto = lir_op_new(
            LIR_OP_TYPE_BEQ,
...
list *compiler_return(module_t *m, closure *c, ast_return_stmt *ast) {
    ...
    if (ast->expr != NULL) {
        list *await = compiler_expr(m, c, *ast->expr, target);
        list_append(operates, await);
...
list *compiler_select_property(module_t *m, closure *c, ast_expr expr, lir_operand *target) {
    ...
    // 计算基值
    lir_operand *base_target = lir_new_empty_operand();
    list_append(operates, compiler_expr(m, c, ast->left, base_target));
...
list *compiler_new_struct(module_t *m, closure *c, ast_expr expr, lir_operand *base_target) {
    ...
    for (int i = 0; i < ast->count; ++i) {
        ...
        list_append(operates, compiler_expr(m, c, struct_property.value, src_temp));
 ...
 * @return
 */
list *compiler_literal(module_t *m, closure *c, ast_literal *literal, lir_operand *target) {
    ...
    switch (literal->type) {
        ...
        default: {
            error_printf(m->compiler_line, "cannot compiler literal->type");
            exit(0);
        }
    }
...
list *compiler_builtin_print(module_t *m, closure *c, ast_call *call, string print_suffix) {
    ...
    for (int i = 0; i < call->actual_param_count; ++i) {
        ...
        list_append(operates, compiler_expr(m, c, ast_param_expr, origin_param_target));
