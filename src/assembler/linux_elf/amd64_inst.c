...
list *linux_elf_amd64_inst_list;

static linux_elf_amd64_inst_t *linux_elf_amd64_inst_new(amd64_asm_inst_t asm_inst) {
    linux_elf_amd64_inst_t *_inst = NEW(linux_elf_amd64_inst_t);
    _inst->data = NULL;
    _inst->count = 0;
    _inst->offset = 0;
...
static void elf_var_operand_rewrite(amd64_asm_operand_t *operand) {
...
static bool elf_is_jmp(amd64_asm_inst_t asm_inst) {
    return asm_inst.name[0] == 'j';
}

static bool elf_is_call(amd64_asm_inst_t asm_inst) {
    return str_equal(asm_inst.name, "call");
}

static uint8_t elf_jmp_reduce_count(amd64_asm_inst_t asm_inst) {
...
static uint8_t elf_jmp_inst_count(amd64_asm_inst_t inst, uint8_t size) {
    if (inst.name[0] != 'j') {
...
static void elf_operand_rewrite_rel(amd64_asm_inst_t inst, amd64_asm_operand_t *operand, int rel_diff) {
    // 已经确定了指令长度，不能再随意修正了
...
void elf_text_inst_build(amd64_asm_inst_t asm_inst, uint64_t *offset) {
    linux_elf_amd64_inst_t *inst = linux_elf_amd64_inst_new(asm_inst);
    ...
    if (rel_operand != NULL) {
        ...
        if (elf_is_call(asm_inst) || elf_is_jmp(asm_inst)) { // TODO 改成 asm is
            ...
            if (table_exist(elf_symbol_table, symbol_operand->name)) {
                ...
                elf_operand_rewrite_rel(asm_inst, rel_operand, rel_diff); // TODO asm
            } else {
    ...
    list_push(linux_elf_amd64_inst_list, inst);
}
...
void elf_confirm_text_rel(string name) {
    if (list_empty(linux_elf_amd64_inst_list)) {
        return;
    }

    list_node *current = list_last(linux_elf_amd64_inst_list); // rear 为 empty 占位
    uint8_t reduce_count = 0;
    // 从尾部像前找, 找到超过 128 即可
    while (true) {
        linux_elf_amd64_inst_t *inst = current->value;
...
void elf_text_inst_list_second_build() {
    ...
    list_node *current = linux_elf_amd64_inst_list->front;
    while (current->value != NULL) {
        linux_elf_amd64_inst_t *inst = current->value;
        ...
        if (symbol != NULL && !symbol->is_rel) {
            ...
            *inst->offset += 1; // 定位到 rel 而不是 inst offset
            ...
            if (symbol == NULL) {
                ...
                symbol->is_rel = true; // 是否为外部引用符号(避免重复添加)
                symbol->is_local = false; // 局部 label 在生成符号表的时候可以忽略
                elf_symbol_insert(symbol); // 插入 linux 维度 elf symbol 表
            }
        }
        current = current->next;
    }
