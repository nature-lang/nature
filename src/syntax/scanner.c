...
token_type scanner_special_char_type() {
    ...
    switch (c) {
        ...
        case '*':
            return TOKEN_STAR;
        case '!':
            return scanner_match('=') ? TOKEN_NOT_EQUAL : TOKEN_NOT;
        case '=':
            return scanner_match('=') ? TOKEN_EQUAL_EQUAL : TOKEN_EQUAL;
        case '<':
            return scanner_match('=') ? TOKEN_LESS_EQUAL : TOKEN_LEFT_ANGLE;
        case '>':
            return scanner_match('=') ? TOKEN_GREATER_EQUAL : TOKEN_RIGHT_ANGLE;
        case '&':
            return scanner_match('&') ? TOKEN_AND_AND : TOKEN_AND;
        case '|':
            return scanner_match('|') ? TOKEN_OR_OR : TOKEN_OR;
        default:
            return -1;
    }
}
