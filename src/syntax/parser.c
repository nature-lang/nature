...
static type_t parser_single_type(module_t *m) {
    ...
    if (parser_is_basic_type(m)) {
        ...
        if (type_token->type == TOKEN_INT || type_token->type == TOKEN_UINT || type_token->type == TOKEN_FLOAT) {
            // 后续 int 会转换为 int64, 所以这里额外记录一下原始类，让 format 更加准确
    ...
    if (parser_is(m, TOKEN_IDENT)) {
        ...
        if (parser_consume(m, TOKEN_LEFT_ANGLE)) {
            ...
            result.def_args = ct_list_new(sizeof(type_t));
            do {
                type_t item = parser_single_type(m);
                ct_list_push(result.def_args, &item);
            } while (parser_consume(m, TOKEN_COMMA));
...
static ast_stmt_t *parser_fndef_stmt(module_t *m, ast_fndef_t *fndef) {
    ...
    if (parser_is_impl_fn(m)) {
        ...
        if (first_token->type == TOKEN_IDENT) {
            impl_type.kind = TYPE_IDENT; // 在没有解析出具体类型之前只能是 ident
            ...
            if (fndef->generics_params) {
                // parser sign type
                parser_must(m, TOKEN_LEFT_ANGLE);
                impl_type.def_args = ct_list_new(sizeof(type_t));
                do {
                    ...
                    ct_list_push(impl_type.def_args, &param_type);
                } while (parser_consume(m, TOKEN_COMMA));
...
static ast_expr_t parser_go_expr(module_t *m) {
    ...
    ast_macro_async_t *go_expr = NEW(ast_macro_async_t);
...
static ast_expr_t parser_macro_async_expr(module_t *m) {
    ast_expr_t result = expr_new(m);
    ast_macro_async_t *async = NEW(ast_macro_async_t);

    parser_must(m, TOKEN_LEFT_PAREN);

    ast_expr_t call_expr = parser_expr(m);
    async->origin_call = call_expr.value;
    async->closure_fn = coroutine_fn_closure(m, &call_expr);
    async->closure_fn_void = coroutine_fn_void_closure(m, &call_expr);

    if (parser_consume(m, TOKEN_COMMA)) {
        async->flag_expr = NEW(ast_expr_t);
        *async->flag_expr = parser_expr(m);
    }
    parser_must(m, TOKEN_RIGHT_PAREN);
