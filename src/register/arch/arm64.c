...
void arm64_reg_init() {
    ...
    x27 = reg_new("x27", 27, LIR_FLAG_ALLOC_INT, QWORD, 0); // current coroutine
    x27->flag |= FLAG(LIR_FLAG_CALLEE_SAVED);
    if (BUILD_OS == OS_DARWIN) {
        x28 = reg_new("x28", 28, LIR_FLAG_ALLOC_INT, QWORD, 0); // Darwin parallel move temporary
    } else {
        x28 = reg_new("x28", 28, LIR_FLAG_ALLOC_INT, QWORD, 26);
    }
    ...
    w16 = reg_new("w16", 16, LIR_FLAG_ALLOC_INT, DWORD, 0); // IP0, 不参与分配
    w17 = reg_new("w17", 17, LIR_FLAG_ALLOC_INT, DWORD, 0);
    ...
    int f_index = BUILD_OS == OS_DARWIN ? 25 : 26;
