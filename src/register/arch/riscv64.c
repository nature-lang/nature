...
void riscv64_reg_init() {
    ...
    r_x0 = reg_new("x0", 0, 0, QWORD, 0); r_zero = r_x0; // zero: Hard-wired zero
    r_x1 = reg_new("x1", 1, 0, QWORD, 0); r_ra = r_x1;   // ra: Return address (Caller)
    r_x2 = reg_new("x2", 2, 0, QWORD, 0); r_sp = r_x2;   // sp: Stack pointer (Callee)
    r_x3 = reg_new("x3", 3, 0, QWORD, 0); r_gp = r_x3;   // gp: Global pointer
    r_x4 = reg_new("x4", 4, 0, QWORD, 0); r_tp = r_x4;   // tp: Thread pointer

    r_x5 = reg_new("x5", 5, LIR_FLAG_ALLOC_INT, QWORD, 1);  // t0: Temporary (Caller)
    r_x6 = reg_new("x6", 6, LIR_FLAG_ALLOC_INT, QWORD, 2);  // t1: Temporary (Caller)
    r_x7 = reg_new("x7", 7, LIR_FLAG_ALLOC_INT, QWORD, 3);  // t2: Temporary (Caller)

    r_x8 = reg_new("x8", 8, 0, QWORD, 0); r_fp = r_x8;   // s0/fp: Saved register/frame pointer (Callee)

    r_x9 = reg_new("x9", 9, LIR_FLAG_ALLOC_INT, QWORD, 4);  // s1: Saved register (Callee)

    r_x10 = reg_new("x10", 10, LIR_FLAG_ALLOC_INT, QWORD, 5); // a0: Function argument/return value (Caller)
    r_x11 = reg_new("x11", 11, LIR_FLAG_ALLOC_INT, QWORD, 6); // a1: Function argument/return value (Caller)
    r_x12 = reg_new("x12", 12, LIR_FLAG_ALLOC_INT, QWORD, 7); // a2: Function argument (Caller)
    r_x13 = reg_new("x13", 13, LIR_FLAG_ALLOC_INT, QWORD, 8); // a3: Function argument (Caller)
    r_x14 = reg_new("x14", 14, LIR_FLAG_ALLOC_INT, QWORD, 9); // a4: Function argument (Caller)
    r_x15 = reg_new("x15", 15, LIR_FLAG_ALLOC_INT, QWORD, 10); // a5: Function argument (Caller)
    r_x16 = reg_new("x16", 16, LIR_FLAG_ALLOC_INT, QWORD, 11); // a6: Function argument (Caller)
    r_x17 = reg_new("x17", 17, LIR_FLAG_ALLOC_INT, QWORD, 12); // a7: Function argument (Caller)

    r_x18 = reg_new("x18", 18, LIR_FLAG_ALLOC_INT, QWORD, 13); // s2: Saved register (Callee)
    r_x19 = reg_new("x19", 19, LIR_FLAG_ALLOC_INT, QWORD, 14); // s3: Saved register (Callee)
    r_x20 = reg_new("x20", 20, LIR_FLAG_ALLOC_INT, QWORD, 15); // s4: Saved register (Callee)
    r_x21 = reg_new("x21", 21, LIR_FLAG_ALLOC_INT, QWORD, 16); // s5: Saved register (Callee)
    r_x22 = reg_new("x22", 22, LIR_FLAG_ALLOC_INT, QWORD, 17); // s6: Saved register (Callee)
    r_x23 = reg_new("x23", 23, LIR_FLAG_ALLOC_INT, QWORD, 18); // s7: Saved register (Callee)
    r_x24 = reg_new("x24", 24, LIR_FLAG_ALLOC_INT, QWORD, 19); // s8: Saved register (Callee)
    r_x25 = reg_new("x25", 25, LIR_FLAG_ALLOC_INT, QWORD, 20); // s9: Saved register (Callee)
    r_x26 = reg_new("x26", 26, LIR_FLAG_ALLOC_INT, QWORD, 21); // s10: Saved register (Callee)
    r_x27 = reg_new("x27", 27, LIR_FLAG_ALLOC_INT, QWORD, 22); // s11: Saved register (Callee)
