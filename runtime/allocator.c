...
static void mcentral_grow(mcentral_t *mcentral) {
    DEBUGF("[runtime.mcentral_grow] spanclass=%d, sizeclass=%d, pages_count=%lu", mcentral->spanclass,
    ...
    DEBUGF("[runtime.mcentral_grow] success, spanclass=%d, base=%lx, alloc_count=%lu, obj_count=%lu", span->spanclass,
           span->base,
...
static mspan_t *cache_span(mcentral_t *mcentral) {
    ...
    HAVE_SPAN:
    assertf(span && span->obj_count - span->alloc_count > 0, "span unavailable, obj_count=%d, alloc_count",
            span->obj_count,
            span->alloc_count);
    return span;
}
...
static mspan_t *mcache_refill(mcache_t *mcache, uint64_t spanclass) {
    DEBUGF("[runtime.mcachine_refill] mcache=%p, spanclass=%lu start", mcache, spanclass);
    mutex_lock(memory->locker);
    DEBUGF("[runtime.mcachine_refill] mcache=%p, spanclass=%lu get clock", mcache, spanclass);
    ...
    if (mspan) {
        ...
        DEBUGF("[runtime.mcachine_refill] mcache=%p, spanclass=%lu uncache span success", mcache, spanclass);
    }
    ...
    DEBUGF("[runtime.mcachine_refill] mcache=%p, spanclass=%lu unlocker", mcache, spanclass);
    return mspan;
}
