...
static void handle_gc_worklist(processor_t *p) {
    DEBUGF("[runtime_gc.handle_gc_worklist] start, processor=%p", p);
    ...
    while (true) {
        if (limit_count >= GC_WORKLIST_LIMIT) {
            DEBUGF("[runtime_gc.handle_gc_worklist] handle_count=%d, will yield", limit_count);
            limit_count = 0;
            aco_yield();
        }
...
static void gc_work() {
    ...
    DEBUGF("[runtime_gc.gc_work] start processor_index=%d, co_id=%p, co_count=%d", p->index, co, p->co_list->count);

    // - share goroutine root and change color black
    LINKED_FOR(p->co_list) {
        coroutine_t *wait_co = LINKED_VALUE();
        DEBUGF("[runtime_gc.gc_work] wait scan_stack co=%p, status=%d, gc_black=%d", wait_co, wait_co->status, wait_co->gc_black);
    ...
    DEBUGF("[runtime_gc.gc_work] handle solo processor gc work list completed, will exit")
}
