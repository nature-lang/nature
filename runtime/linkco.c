...
linkco_t *rti_acquire_linkco() {
    ...
    if (p->linkco_count == 0) {
        ...
        while (p->linkco_count < (P_LINKCO_CACHE_MAX / 2) && global_linkco_cache != NULL) {
            ...
            linkco->next = NULL;
            DEBUGF("[rti_acquire_linkco] from global cache linkco %p, prev %p, next %p to p_index: %d, count %d",
    ...
    DEBUGF("[rti_acquire_linkco] p: %d, count %d linkco from cache  %p, prev %p, next %p",
...
void rti_release_linkco(linkco_t *linkco) {
    ...
    if (p->linkco_count == P_LINKCO_CACHE_MAX) {
        ...
        while (p->linkco_count > (P_LINKCO_CACHE_MAX / 2)) {
            ...
            DEBUGF("[rti_release_linkco] release linkco to p %d, count %d ,linkco %p pcache to global", p->index, p->linkco_count, temp);
    ...
    //    p->linkco_cache[p->linkco_count++] = linkco;
    rti_write_barrier_ptr(&p->linkco_cache[p->linkco_count++], linkco, true);
    DEBUGF("[rti_release_linkco] release linkco to p %d, count %d write_barrier from cache %p", p->index, p->linkco_count, linkco);
}
