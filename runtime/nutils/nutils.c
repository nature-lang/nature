        ...
        switch (_kind) {                                                                                                               \
            ...
            case TYPE_UINT8:                                                                                                           \
                *(uint8_t *)output_ref = (uint8_t)_input_value;                                                                        \
                return;                                                                                                                \
            default:                                                                                                                   \
                assertf(false, "cannot convert %s to %s type", type_kind_str[input_rtype->kind], type_kind_str[output_rtype->kind]);   \
                exit(1);                                                                                                               \
        }                                                                                                                              \
    }
...
int64_t iterator_next_value(void *iterator, uint64_t rtype_hash, int64_t cursor, void *value_ref) {
    ...
    if (iterator_rtype->kind == TYPE_VEC || iterator_rtype->kind == TYPE_STRING) {
        n_vec_t *list = iterator;
        assertf(list->element_rtype_hash, "list element rtype hash is empty, ptr: %p, len: %lu, cap: %lu, data: %p", list, list->length,
...
rtype_t *gc_rtype(type_kind kind, uint32_t count, ...) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        str = safe_str_connect_free(str, safe_itoa(arg_kind));
    }
    va_end(valist);
    ...
    rtype->gc_bits = safe_malloc_gc_bits(count * POINTER_SIZE);
