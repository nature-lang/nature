...
static inline n_array_t *rti_array_new(rtype_t *element_rtype, uint64_t length) {
    ...
    DEBUGF("[rti_array_new] ele_sz=%lu(rtype_stack_size=%lu),ele_kind=%s(n_gc=%d),len=%lu", element_rtype->size,
    ...
    DEBUGF(
            ...
            addr, element_rtype->size, type_kind_str[element_rtype->kind], element_rtype->last_ptr, rtype.size,
            length,
