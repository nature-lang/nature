...
void *fn_new(addr_t fn_addr, envs_t *envs) {
    ...
    runtime_fn_t *fn_runtime = runtime_rtype_malloc(sizeof(runtime_fn_t), fn_rtype);
...
envs_t *env_new(uint64_t length) {
    ...
    envs_t *envs = runtime_rtype_malloc(sizeof(envs_t), envs_rtype);
...
void env_assign(envs_t *envs, uint64_t item_rtype_hash, uint64_t env_index, addr_t stack_addr) {
    ...
    if (!upvalue) {
        ...
        upvalue = runtime_rtype_malloc(sizeof(upvalue_t), upvalue_rtype);
