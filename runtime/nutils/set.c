...
static void set_grow(n_set_t *m) {
    ...
    m->hash_table = runtime_gc_malloc(sizeof(int64_t) * m->capacity, NULL);
...
n_set_t *set_new(uint64_t rtype_hash, uint64_t key_index) {
    ...
    n_set_t *set_data = runtime_gc_malloc(set_rtype->size, set_rtype);
    ...
    set_data->hash_table = runtime_gc_malloc(sizeof(uint64_t) * set_data->capacity, NULL);
