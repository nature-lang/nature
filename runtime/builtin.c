...
static void print_arg(n_union_t *arg) {
    ...
    if (arg->rtype->kind == TYPE_STRING) {
        ...
        DEBUGF("[runtime.print_arg] string=%p, length=%lu, data=%s, data_str_len=%lu", s, s->length, s->data,
               s->length);
...
void print(n_vec_t *args, bool with_space) {
    ...
    TDEBUGF("[runtime.print] memory_list_t base=%p,length=%lu, array_data_base=%lx, element_size=%lu", args,
            args->length, base,
            element_size);
    ...
    for (int i = 0; i < args->length; ++i) {
        ...
        TDEBUGF("[runtime.print] arg i=%d, p=0x%lx, p_value=%p ", i, p, *value);
        print_arg(*value);
