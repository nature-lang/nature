...
//    DEBUGF("[runtime.co_preempt_yield] start")
...
//    DEBUGF("[runtime.co_preempt_yield.thread_locker] mutex_unlock, p_index_%d=%d, co=%p, will yield",
//           p->share, p->index, co);
//
    co_yield_runnable(p, co);

//    DEBUGF("[runtime.co_preempt_yield] end, will return, p_index_%d=%d co=%p, p->co=%p", p->share, p->index, co,
//           p->coroutine);
}
...
void coroutine_resume(processor_t *p, coroutine_t *co) {
    ...
    // 将 RIP 指针移动用户代码片段中
    DEBUGF("[runtime.coroutine_resume] aco_resume will start, co=%p, aco=%p, p->co_started_at=%lu", co, co->aco,
           p->co_started_at);
    ...
    DEBUGF("[runtime.coroutine_resume] aco_yield completed, co=%p, aco=%p, status=%d, time=%lu ms", co, co->aco,
           co->status, time);
}
    ...
    while (true) {
        ...
        if (processor_get_stw()) {
            ...
            while (processor_get_stw()) {
                DEBUGF("[runtime.processor_run] stw loop....")
                usleep(WAIT_MID_TIME * 1000); // 每 50ms 检测一次 STW 是否解除
            }

            p->safe_point = false;
            DEBUGF("[runtime.processor_run] p_index=%d, stw completed, set safe_point=false, share=%d", p->index,
                   p->share);
        }
        ...
        while (true) {
            ...
            DEBUGF("[runtime.processor_run] will handle coroutine, p_index=%d, co=%p, status=%d, share=%d", p->index,
                   co, co->status,
                   p->share);
            ...
            DEBUGF("[runtime.processor_run] coroutine resume completed, p_index=%d, co=%p, status=%d, share=%d",
        ...
        if (!p->share) {
            ...
            if (p->coroutine->status == CO_STATUS_DEAD) {
                DEBUGF("[runtime.processor_run] solo processor exit, p_index=%d, co=%p, status=%d, share=%d", p->index,
                       p->coroutine,
