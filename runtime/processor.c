...
static void coroutine_run(processor_t* p, coroutine_t* c) {
    assertf(c->aco, "coroutine 创建的时候就需要初始化指向了");

    c->status = CO_STATUS_RUNNING;

    aco_resume(c->aco);

    if (c->status != CO_STATUS_DEAD) {
        c->status = CO_STATUS_WAITING;
    }
}

static void* exclusive_processor_run(void* arg) {
    int i = *(int*)arg;
    free(arg);
    processor_t* p = &exclusive_processor_list[i];
    ...
    while (true) {
        ...
        if (co->status == CO_STATUS_RUNNABLE) {
            coroutine_run(p, co);
        }
