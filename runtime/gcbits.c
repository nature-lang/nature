...
gc_bits* gcbits_new(uintptr_t bytes_size) {
    ...
    if (p != NULL) {
        ...
        gc_bits_arenas.free_list = fresh->next;
...
gc_bits* gcbits_try_alloc(gc_bits_arena_t* arena, uintptr_t bytes_size) {
    ...
    arena->free_index += bytes_size;
    if (arena->free_index > GCBITS_CHUNK_PAYLOAD_BYTES) {
        return NULL;
    }

    uintptr_t start = arena->free_index - bytes_size;
    return &arena->bits[start];
}
...
gc_bits_arena_t* gcbits_arena_new() {
    ...
    if (gc_bits_arenas.free_list == NULL) {
        ...
        // clear result
        memset(result, 0, GCBITS_CHUNK_PAYLOAD_BYTES);
    }

    result->next = NULL;
    result->free_index = 0; // 目前总是从 0 开始的， 除非 gc_bits_arena 添加了其他结构
