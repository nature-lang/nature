        ...
        size_t *size_read)
{
    ...
    if (parser->input.str.current == parser->input.str.end) {
        *size_read = 0;
        return 1;
    }

    if (size > (size_t)(parser->input.str.end
                - parser->input.str.current)) {
        size = parser->input.str.end - parser->input.str.current;
    }

    memcpy(buffer, parser->input.str.current, size);
    parser->input.str.current += size;
    *size_read = size;
    return 1;
}
        ...
        const unsigned char *input, size_t size)
{
    ...
    parser->input.str.start = input;
    parser->input.str.current = input;
    parser->input.str.end = input+size;
}
...
yaml_string_write_handler(void *data, unsigned char *buffer, size_t size)
{
  yaml_emitter_t *emitter = (yaml_emitter_t *)data;
    ...
    if (emitter->output.str.size - *emitter->output.str.size_written
            < size) {
        memcpy(emitter->output.str.buffer
                + *emitter->output.str.size_written,
                buffer,
                emitter->output.str.size
                - *emitter->output.str.size_written);
        *emitter->output.str.size_written = emitter->output.str.size;
        return 0;
    }

    memcpy(emitter->output.str.buffer
            + *emitter->output.str.size_written, buffer, size);
    *emitter->output.str.size_written += size;
    return 1;
}
        ...
        unsigned char *output, size_t size, size_t *size_written)
{
    ...
    emitter->output.str.buffer = output;
    emitter->output.str.size = size;
    emitter->output.str.size_written = size_written;
    *size_written = 0;
}
