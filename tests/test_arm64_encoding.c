...
static void test_basic() {
    ...
    TEST_EQ(inst, 0x40, 0xD0, 0x3B, 0xD5);

    // SUB sp, sp, #32
    inst = ARM64_INST(R_SUB, ARM64_REG(sp), ARM64_REG(sp), ARM64_IMM(32));
    TEST_EQ(inst, 0xFF, 0x83, 0x00, 0xD1);

    // ADRP x8, :got:foo
    // 注意：ADRP的编码依赖于具体的地址，这里使用占位符
    inst = ARM64_INST(R_ADRP, ARM64_REG(x8), ARM64_SYM("hello", true, 0, 0)); // 使用0作为占位符
    TEST_EQ(inst, 0x08, 0x00, 0x00, 0x90);

    // LDR x8, [x8, :got_lo12:foo]
    inst = ARM64_INST(R_LDR, ARM64_REG(x8), ARM64_INDIRECT(x8, 0, 0, 8)); // 假设0表示无偏移
    TEST_EQ(inst, 0x08, 0x01, 0x40, 0xF9);

    //  bl test 类似于 amd64 中的 call test
    inst = ARM64_INST(R_BL, ARM64_SYM("test", true, 0, 0));
    TEST_EQ(inst, 0x00, 0x00, 0x00, 0x94);

    // bl offset
    inst = ARM64_INST(R_BL, ARM64_SYM("test", true, 0x20, 0));
    TEST_EQ(inst, 0x08, 0x00, 0x00, 0x94);

    inst = ARM64_INST(R_BL, ARM64_IMM(0x10));
    TEST_EQ(inst, 0x04, 0x00, 0x00, 0x94);

    // beq 测试
    inst = ARM64_INST(R_BEQ, ARM64_SYM("symbol", true, 0, 0));
    TEST_EQ(inst, 0x00, 0x00, 0x00, 0x54);

    inst = ARM64_INST(R_BEQ, ARM64_IMM(0x10));
    TEST_EQ(inst, 0x80, 0x00, 0x00, 0x54);

    // strb 和 str 比较测试
    inst = ARM64_INST(R_STR, ARM64_REG(w7), ARM64_INDIRECT(x29, -1, 0, 8)); // 假设0表示无偏移
    TEST_EQ(inst, 0xA7, 0xF3, 0x1F, 0xB8);

    inst = ARM64_INST(R_STRB, ARM64_REG(w7), ARM64_INDIRECT(x29, -1, 0, 1)); // 假设0表示无偏移
    TEST_EQ(inst, 0xA7, 0xF3, 0x1F, 0x38);

    // strb w2, [x1, #8]
    inst = ARM64_INST(R_STRB, ARM64_REG(w2), ARM64_INDIRECT(x1, 8, 0, 1));
    TEST_EQ(inst, 0x22, 0x20, 0x00, 0x39);

    // strh w2, [x1, #8]
    inst = ARM64_INST(R_STRH, ARM64_REG(w2), ARM64_INDIRECT(x1, 8, 0, 1));
    TEST_EQ(inst, 0x22, 0x10, 0x00, 0x79);

    // str w2, [x1, #8]
    inst = ARM64_INST(R_STR, ARM64_REG(w2), ARM64_INDIRECT(x1, 8, 0, 1));
    TEST_EQ(inst, 0x22, 0x08, 0x00, 0xB9);

    // str x2, [x1, #8]
    inst = ARM64_INST(R_STR, ARM64_REG(x2), ARM64_INDIRECT(x1, 8, 0, 1));
    TEST_EQ(inst, 0x22, 0x04, 0x00, 0xf9);

    // ldrb w2, [x1, #8]
    inst = ARM64_INST(R_LDRB, ARM64_REG(w2), ARM64_INDIRECT(x1, 8, 0, 1));
    TEST_EQ(inst, 0x22, 0x20, 0x40, 0x39);

    // ldrh w2, [x1, #8]
    inst = ARM64_INST(R_LDRH, ARM64_REG(w2), ARM64_INDIRECT(x1, 8, 0, 1));
    TEST_EQ(inst, 0x22, 0x10, 0x40, 0x79);

    // ldr w2, [x1, #8]
    inst = ARM64_INST(R_LDR, ARM64_REG(w2), ARM64_INDIRECT(x1, 8, 0, 1));
    TEST_EQ(inst, 0x22, 0x08, 0x40, 0xB9);

    // ldr x2, [x1, #8]
    inst = ARM64_INST(R_LDR, ARM64_REG(x2), ARM64_INDIRECT(x1, 8, 0, 1));
    TEST_EQ(inst, 0x22, 0x04, 0x40, 0xf9);

    // str d0, [x0]  - 存储64位浮点数，无偏移
    inst = ARM64_INST(R_STR, ARM64_REG(d0), ARM64_INDIRECT(x0, 0, 0, 8));
    TEST_EQ(inst, 0x00, 0x00, 0x00, 0xFD);

    // str s1, [x0, #8]  - 存储32位浮点数，偏移8字节
    inst = ARM64_INST(R_STR, ARM64_REG(s1), ARM64_INDIRECT(x0, 8, 0, 4));
    TEST_EQ(inst, 0x01, 0x08, 0x00, 0xBD);

    // str d0, [x0, #16]  - 存储64位浮点数，偏移16字节
    inst = ARM64_INST(R_STR, ARM64_REG(d0), ARM64_INDIRECT(x0, 16, 0, 8));
    TEST_EQ(inst, 0x00, 0x08, 0x00, 0xFD);

    // 测试数字 mov
    inst = ARM64_INST(R_MOV, ARM64_REG(w0), ARM64_IMM(-1238));
    ...
    // 测试 eor
    inst = ARM64_INST(R_EOR, ARM64_REG(w0), ARM64_REG(w1), ARM64_REG(w2));
    TEST_EQ(inst, 0x20, 0x00, 0x02, 0x4A);

    // 测试 mvn
    inst = ARM64_INST(R_MVN, ARM64_REG(x0), ARM64_REG(x1)); // E00321AA
    TEST_EQ(inst, 0xE0, 0x03, 0x21, 0xAA);

    // 测试 fmov d1, xzr
    inst = ARM64_INST(R_FMOV, ARM64_REG(d1), ARM64_REG(xzr)); // E103679E
