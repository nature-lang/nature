...
fn align_up(int offset):int {
    return (offset + ALIGN - 1) & ~(ALIGN - 1)
}

// allocate a new block with the given capacity
fn block_new(int capacity):ptr<block_t> {
...
fn block_data(ptr<block_t> b):anyptr {
...
fn block_alloc(ptr<block_t> b, int size):anyptr {
...
// allocatable interface impl: allocate `size` bytes from the arena
fn arena_t.alloc(*self, int size):anyptr {
    if size <= 0 {
        panic('arena: alloc invalid size')
    }
    ...
    if p == null {
        ...
        if p == null {
            panic('arena: alloc failed after expanding')
        }
    }
...
// allocatable interface impl: deallocate memory at `p`
// arena only supports tail-free optimization
fn arena_t.dealloc(*self, anyptr p) {
    ...
    // otherwise no-op — arena does not support freeing arbitrary allocations
}
...
// convenience generic helper: free a typed pointer
fn arena_t.free<T>(*self, ptr<T> p) {
    self.dealloc(p as anyptr)
}

fn arena_t.reset(*self) {
